...
Create a database named `draftline`.
...
Celery runs the background work Draftline depends on: Gorgias backfill, website
crawls, source sync, and webhook processing. Without a worker these are queued
in Redis and never execute, so connections stay `PENDING` and no data appears.

`make dev` starts a worker alongside Django and Vite. Set `RUN_CELERY=0` (or use
`make dev-no-celery`) to skip it, and `CELERY_QUEUES` to change which queues it
consumes.

To run a worker on its own:
...
`docker compose up -d` is running first.

Queues in use: `celery` (default), `backfill`, `sync`, `drafting`, `deltas`,
`maintenance`. A worker only picks up the queues named in `-Q`, so a worker
started without `backfill` will leave backfill jobs sitting in Redis.

...
class TeamContextRestoreTest(TestCase):
    def test_request_restores_enclosing_team_context(self):
        # Regression test: TeamsMiddleware must restore the team context that was active
        # before the request, not clear it.
        team = Team.objects.create(name="Red Sox", slug="sox")
        token = set_current_team(team)
        self.addCleanup(unset_current_team, token)

        self.client.get(reverse("web:home"))

        self.assertEqual(get_current_team(), team)

    async def test_async_request_does_not_error(self):
        # Regression test: under ASGI, sync middleware hooks run in different executor
        # contexts, so the context token can't be used to reset and must be handled gracefully.
        response = await self.async_client.get(reverse("web:home"))
...
class TeamsAuthTest(TestCase):
    ...
    def test_nonexistent_team_slug_returns_404(self):
        self._login(self.sox_admin)
        response = self.client.get("/a/nonexistent-team/team/")
        self.assertEqual(404, response.status_code)

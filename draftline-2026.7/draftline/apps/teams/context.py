...
def unset_current_team(token: Token | None = None):
    ...
    if token is None:
        ...
        try:
            ...
            # The token was created in a different Context — e.g. under ASGI, where sync
            # middleware hooks run in separate executor contexts. Restoring the previous
            # value is impossible in that case, so fall back to clearing.
            _context.set(None)

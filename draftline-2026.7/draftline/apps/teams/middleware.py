...
class TeamsMiddleware:
    ...
    def __call__(self, request):
        try:
            ...
            unset_current_team(getattr(request, "_team_context_token", None))
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        # Note: a single underscore is used because a dunder prefix would trigger name mangling
        # inside the class body, storing the token under a different name than getattr() looks up.
        request._team_context_token = set_current_team(request.team)

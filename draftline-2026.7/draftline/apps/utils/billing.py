...
def get_discounted_price(amount: decimal.Decimal, coupon: Coupon) -> decimal.Decimal:
    if coupon.amount_off:
        return max(amount - coupon.amount_off, 0)
    elif coupon.percent_off:
        # percent_off comes out of stripe_data as an int/float, which can't be mixed with Decimal
        return amount * (1 - (decimal.Decimal(str(coupon.percent_off)) / 100))
    return amount
...
def get_friendly_currency_amount(price: Price, currency: str | None = None) -> str:
    ...
    if currency != price.currency:
        ...
        amount = int(float(price.unit_amount_decimal))

...
class SubscriptionSerializerTests(TestCase):
    """Test that the SubscriptionSerializer handles dj-stripe 2.10+ integer timestamps."""

    @classmethod
    def setUpTestData(cls):
        cls.user = CustomUser.objects.create(username="serializer_test@example.com")
        cls.team = Team.objects.create(name="SerializerTeam", slug="serializer-team")
        cls.team.members.add(cls.user, through_defaults={"role": ROLE_ADMIN})
        cls.subscription = create_subscription_for_team(cls.team, MOCK_ACTIVE_PRODUCTS[0], metered=True)

    def test_serializer_handles_integer_timestamps(self):
        """SubscriptionSerializer should not crash when datetime fields are unix timestamps (integers)."""
        wrapped = SubscriptionWrapper(self.subscription)
        serializer = SubscriptionSerializer(wrapped)
        # This will raise AttributeError: 'int' object has no attribute 'utcoffset'
        # if DateTimeField can't handle integer timestamps
        data = serializer.data
        self.assertIn("current_period_start", data)
        self.assertIn("current_period_end", data)
        # Verify the values are valid datetime strings, not integers

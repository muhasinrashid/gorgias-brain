...
from .exceptions import InvalidPriceError, SubscriptionConfigError
...
def subscription_is_trialing(subscription: Subscription) -> bool:
    trial_end = convert_tstamp(subscription.trial_end)
    return subscription.status == SubscriptionStatus.trialing and trial_end is not None and trial_end > timezone.now()
...
def validate_stripe_price_id(price_id: str) -> None:
    """Validate that a price ID matches the expected Stripe format."""
    if not re.match(r"^(price|plan)_[A-Za-z0-9]+$", price_id):
        raise InvalidPriceError(f"Invalid Stripe price ID: {price_id!r}")


def create_stripe_checkout_session(
    subscription_holder: Team, stripe_price_id: str, user: CustomUser
) -> stripe.checkout.Session:
    validate_stripe_price_id(stripe_price_id)
...
def cancel_subscription(subscription_id: str):
    try:
        ...
        if e.code != "resource_missing":
            logger.error("Error deleting Stripe subscription: %s", e.user_message)
    else:

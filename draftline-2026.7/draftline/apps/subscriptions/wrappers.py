...
from stripe import InvalidRequestError, Invoice
...
class SubscriptionWrapper:
    ...
    def start_date(self) -> datetime:
        """Convert start_date from unix timestamp (via stripe_data) to a datetime."""
        return convert_tstamp(self.subscription.start_date) or self.subscription.created

    @property
    def current_period_end(self) -> datetime | None:
        """Convert current_period_end from unix timestamp (via stripe_data) to a datetime."""
        return convert_tstamp(self.subscription.current_period_end)

    @property
    def trial_end(self) -> datetime | None:
        """Convert trial_end from unix timestamp (via stripe_data) to a datetime."""
        return convert_tstamp(self.subscription.trial_end)
    ...
    def is_metered(self) -> bool:
        ...
        """
        return self.items.filter(price__stripe_data__recurring__usage_type="metered").exists()

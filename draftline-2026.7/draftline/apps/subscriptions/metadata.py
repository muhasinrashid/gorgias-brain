        ...
        if self.product:
            try:
                return self.product.prices.get(
                    stripe_data__recurring__interval=interval,
                    stripe_data__recurring__interval_count=1,
                    active=True,
                )
...
def get_help_text_for_interval(interval: str) -> str:
    return {

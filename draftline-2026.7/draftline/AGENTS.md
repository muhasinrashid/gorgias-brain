...
- Product / engineering spec: `docs/PRODUCT_SPEC.md` (read before building features).
- Stage 0 recon: `docs/STAGE0_RECON.md`. External API quirks: `docs/EXTERNAL_API_NOTES.md`.
- Session notes: `docs/WORKLOG.md`. Junior onboarding: `docs/DEVELOPER_HANDOFF.md`.
- Feature gates must read `TeamEntitlement` (`apps.billing`) — never Stripe/dj-stripe.
- Domain models are team-scoped via `BaseTeamModel` + `for_team`.
- Gorgias may only create **internal notes** and tags — never customer-visible messages.
...
- When calling the Stripe API directly (not through dj-stripe), always use `get_stripe_module()` from `apps.utils.billing`
  instead of `import stripe`. This ensures `stripe.api_key` is properly set.
